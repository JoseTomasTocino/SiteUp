...
Así, en la implementación se itera por las respuestas, comprobando si alguna de
...
\textit{\nameref{subsec:chequeoHttp}}, las respuestas a las peticiones HTTP
...
En este caso, el modelo \textit{PingCheck} es, a su vez, un tipo de
...
\begin{figure}[hbtp]
  \centering
  \includegraphics[width=0.7\textwidth]{6_implementacion/diagrama-er-2}
...
obtener todos los grupos de chequeos:

\begin{minted}{python}
grupos = CheckGroup.objects.all()
\end{minted}

Para obtener el grupo llamado \textit{Personal}:

\begin{minted}{python}
grupo = CheckGroup.objects.get(title="Personal") 
\end{minted}

Es posible embeber condiciones algo más avanzadas. Por ejemplo, para obtener los
chequeos que se ejecutan cada 3 minutos o menos:

\begin{minted}{python}
chequeos_frecuentes = PingCheck.objects.filter(check_interval__lte=3)
\end{minted}
...
\begin{minted}{python}
grupo = CheckGroup.objects.get(title="Personal") 
chequeos_personales = grupo.pingcheck_set.all()

chequeo = PingCheck.objects.get(title="Web personal")
grupo = PingCheck.group
\end{minted}
...
ejemplo, si borramos un grupo, todos los chequeos del grupo son borrados también
de forma automática.
...
Traceback (most recent call last):
 ...
DoesNotExist: PingCheck matching query does not exist.
\end{minted}
...
introducir una dirección de correo inválida para un usuario, el sistema nos alertará:

\begin{minted}{pycon}
>>> usuario = User()
>>> usuario.username = "Fab"
>>> usuario.email = "invalido"
>>> usuario.full_clean()
Traceback (most recent call last):
 ...
ValidationError: {'email': [u'Enter a valid email address.']}
\end{minted}
...
atributo \textit{description} al modelo \textit{CheckGroup}, éste quedaría
así:
...
$ python manage.py schemamigration aplicacion --auto  
 + Added field description on main.CheckGroup
Created 0002_auto__add_field_group_description.py. 

$ python manage.py migrate aplicacion
Running migrations for main:
 - Migrating forwards to 0002_auto__add_field_group_description.
 > main:0002_auto__add_field_group_description
